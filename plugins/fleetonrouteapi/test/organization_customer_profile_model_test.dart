//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @ dart=2.8

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: lines_longer_than_80_chars

import 'package:fleetonrouteapi/api.dart';
import 'package:test/test.dart';

// tests for OrganizationCustomerProfileModel
void main() {
  final instance = OrganizationCustomerProfileModel();

  group('test OrganizationCustomerProfileModel', () {
    // String type
    test('to test the property `type`', () async {
      // TODO
    });

    // String id
    test('to test the property `id`', () async {
      // TODO
    });

    // String userProfileId
    test('to test the property `userProfileId`', () async {
      // TODO
    });

    // StripeCustomerProfileModel stripe
    test('to test the property `stripe`', () async {
      // TODO
    });

    // OfflineCustomerProfileModel offline
    test('to test the property `offline`', () async {
      // TODO
    });

    // String organizationId
    test('to test the property `organizationId`', () async {
      // TODO
    });
  });
}
